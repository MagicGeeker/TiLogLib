...
#define synchronized_remove_ref(x) typename std::remove_reference<x>::type
...
#define synchronized(mtx)                                                                                                                  \
	for (synchronized_unique_lock<synchronized_remove_ref(decltype(mtx))> lk_0(mtx), *_lk_ = &lk_0; _lk_ != nullptr; _lk_ = nullptr)
#define synchronized_u(lk, mtx)                                                                                                            \
	for (synchronized_unique_lock<synchronized_remove_ref(decltype(mtx))> lk(mtx), *_lk_ = &lk; _lk_ != nullptr; _lk_ = nullptr)
#define synchronized_s(slk, smtx)                                                                                                          \
	for (synchronized_shared_lock<synchronized_remove_ref(decltype(mtx))> slk(smtx), *_slk_ = &slk; _slk_ != nullptr; _slk_ = nullptr)

#define synchronized_break_and_unlock break
#define synchronized_return_and_unlock return
